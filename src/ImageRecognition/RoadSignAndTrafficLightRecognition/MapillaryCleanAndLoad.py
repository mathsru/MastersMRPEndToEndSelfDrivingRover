#Alex Eliseev
#The first dataset used for image recognition was the S2TLD, this script adds the Mapillary annotated dataset into the training, testing and validation pool.
...
JSONS_MAPILLARY = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "raw" / "MapillaryDataset" / "mtsd_v2_fully_annotated" / "annotations"
IMAGES_MAPILLARY = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "raw" / "MapillaryDataset" / "images"

#First doing so for training images + labels from Mapillary
TRAIN_SPLIT_MAPILLARY_TXT = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "raw" / "MapillaryDataset" / "mtsd_v2_fully_annotated" / "splits" / "train.txt"
OUTPUT_LABEL_TRAIN_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "train" / "labels" #Appending yolo labels to this folder
OUTPUT_IMAGE_TRAIN_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "train" / "images" #Appending images for matching json/yolo labels to this folder
...
#Repeating same process except for test split of the mapillary dataset
#First doing so for training images + labels from Mapillary
#In mapillary the test images dont have labels, have to basically go in and just by observation see if its detecting stuff properly it is what it is.
TEST_SPLIT_MAPILLARY_TXT = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "raw" / "MapillaryDataset" / "mtsd_v2_fully_annotated" / "splits" / "test.txt"
OUTPUT_LABEL_TEST_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "test" / "labels" #Appending yolo labels to this folder
OUTPUT_IMAGE_TEST_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "test" / "images" #Appending images for matching json/yolo labels to this folder

#Loop opens text file listing train JSONS and processes each one saving them + their class lables in YOLO format and converted to my classes as txt
#to a train/images+labels folders.
with open(TEST_SPLIT_MAPILLARY_TXT, "r") as f:
    for line in f:
        json_id = line.strip()
        ...
        # Mapillary images may not match extension, so try common ones
        found_image = None

        for ext in [".jpg", ".jpeg", ".png"]:
            candidate = IMAGES_MAPILLARY / f"{json_id}{ext}"
            if candidate.exists():
                found_image = candidate
                break

        if found_image:
            destination = OUTPUT_IMAGE_TEST_FOLDER / found_image.name
            shutil.copy2(found_image, destination)
        else:
            print(f"Missing image for: {json_id}")

#And finally for the validation split
VAL_SPLIT_MAPILLARY_TXT = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "raw" / "MapillaryDataset" / "mtsd_v2_fully_annotated" / "splits" / "val.txt"
OUTPUT_LABEL_VAL_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "val" / "labels" #Appending yolo labels to this folder
OUTPUT_IMAGE_VAL_FOLDER = PROJECT_ROOT / "Data" / "SignAndTrafficLights" / "val" / "images" #Appending images for matching json/yolo labels to this folder

#Loop opens text file listing train JSONS and processes each one saving them + their class lables in YOLO format and converted to my classes as txt
#to a train/images+labels folders.
with open(VAL_SPLIT_MAPILLARY_TXT, "r") as f:
    for line in f:
        json_id = line.strip()
        ...
        json_name = json_path.stem #Getting JSON name
        output_txt_path = OUTPUT_LABEL_VAL_FOLDER / f"{json_name}.txt" #Txt location to which script is writing labels for image
        ...
        if source_image.exists():
            ...
            destination_image = OUTPUT_IMAGE_VAL_FOLDER / source_image.name
