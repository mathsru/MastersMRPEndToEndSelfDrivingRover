...
class TrainingGymEnvSingleLight(gym.Env):
    def __init__(self):
        super().__init__()
        ...
        # Always start with a red light
        self.light_timer = 0
        self.light_phase = 2              # 0 = Green, 2 = Red
        self.current_cycle_duration = 30.0
    ...
    def step(self, action):
        if time.time() - self.episode_start_time > 100:
            print("Episode timeout")
            observation = self.get_state()

            reward = -300 #Big penalty for timeout and trying to farm the red infinently
        ...
        if self.vehicle.get_location().distance(self.end_location) < 20:
            ...
            self.episode_number += 1
            return observation, reward, True, False, {}
        else: #Letting the model to brake or do nothing
            #print(action)
            if action == 0: #Setting the brake value of the control to 0/releasing the brake pedal, moving with default speed
                ...
                self.action = 0
            elif action == 1: #Hitting the brake pedal, applying brakes
                control = carla.VehicleControl(
                    throttle=0.0,
                    brake=1.0,
                    steer=0.0,
    ...
    def calculate_reward(self,observation):
        ...
        reward = 0
        speeddrop = self.previous_speed - speed
    ...
    #Function controls the lights to change green -> red -> green every 10 seconds
